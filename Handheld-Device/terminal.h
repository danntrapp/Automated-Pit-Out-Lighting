...
uint8_t current_tx_power = 13; //default
...
void argument_mapping(char * arguments[MAX_ARGS], uint8_t num_args){
    ...
    if (0 == strcmp(arguments[0], "help")){ 
      ...
      if (0 == strcmp(arguments[1], "override")){
      ...
      if (0 == strcmp(arguments[1], "green")){
        
        format_terminal_for_new_entry();
        Serial.printf("Triggering green button press.\n");
        format_new_terminal_entry();
      ...
      if (0 == strcmp(arguments[1], "red")){
        
        format_terminal_for_new_entry();
        Serial.printf("Triggering red button press.\n");
        format_new_terminal_entry();
        ...
        format_terminal_for_new_entry();
        serial.print("Valid options are: override.\n");
        format_new_terminal_entry();
      }
    ...
    else if (0 == strcmp(arguments[0], "soc")){
      extern TaskHandle_t soc_monitoring_task_handle;
      ...
      int soc = atoi(arguments[1]);
      if ((soc >= 0) && (soc <= 100)){
        format_terminal_for_new_entry();
        serial.printf("Set SOC to %d%...\n", soc);
        battery_soc = soc;
        format_new_terminal_entry();
        vTaskSuspend(soc_monitoring_task_handle);
      }
      else if(soc == -1){
        format_terminal_for_new_entry();
        serial.print("Resuming task...\n");
        format_new_terminal_entry();
        vTaskResume(soc_monitoring_task_handle);
      }
    }
