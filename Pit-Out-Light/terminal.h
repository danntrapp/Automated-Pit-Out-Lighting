...
extern TaskHandle_t request_task_handle;
...
subsystem term_destination_device = HHD; //default is POL
...
//Inputs: a (first integer operand) & b (second integer operand)
//Outputs: returns a < b
...
//Name: suspend_all_tasks
//Purpose: Suspends all tasks (besides the terminal task). 
//Inputs: None  
//Outputs: None
void suspend_all_tasks(){

    vTaskSuspend(ping_task_handle);
...
//Purpose: Resumes all tasks (besides the terminal task)
//Inputs: None  
//Outputs: None
void resume_all_tasks(){
    
    vTaskResume(ping_task_handle);
...
void argument_mapping(char * arguments[MAX_ARGS], uint8_t num_args){
    ...
    else if (0 == strcmp(arguments[0], "trigger")){

      if (num_args < 2){
        format_terminal_for_new_entry();
        serial.print("Incorrect number of arguments entered for trigger command.\n");
        format_new_terminal_entry();
      }
        
      else if (0 == strcmp(arguments[1], "override")){
        
        format_terminal_for_new_entry();
        serial.printf("Triggering override.\n");
        format_new_terminal_entry();

        trigger_flag = 1;
        comms.packet_contents.request = OVERRIDE_START;
        vTaskResume(rx_task_handle);
        

      }

      else if (0 == strcmp(arguments[1], "red")){

        format_terminal_for_new_entry();
        serial.printf("Triggering red light toggle.\n");
        format_new_terminal_entry();

        trigger_flag = 1;
        comms.packet_contents.request = RED;
        comms.packet_contents.payload = digitalRead(RED_LIGHT_PIN) ^ 1;
        vTaskResume(rx_task_handle);
      }

      else if (0 == strcmp(arguments[1], "green")){

        format_terminal_for_new_entry();
        serial.printf("Triggering green light toggle.\n");
        format_new_terminal_entry();

        trigger_flag = 1;
        comms.packet_contents.request = GREEN;
        comms.packet_contents.payload = digitalRead(GREEN_LIGHT_PIN) ^ 1;
        vTaskResume(rx_task_handle);
        ...
        format_terminal_for_new_entry();
        serial.print("Valid options are: green, override, and red.\n");
        format_new_terminal_entry();
      }
      
      else { 
        format_terminal_for_new_entry();
        serial.print("Invalid entry for trigger command.\n");
        format_new_terminal_entry();
      }
